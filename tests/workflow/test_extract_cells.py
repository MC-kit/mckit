...
@pytest.mark.parametrize(
    "universe,comment,expected,assc",
    [
        ("universe1.i", "sphere", [1], False),
        ("universe1.i", "sphere", [1, 3], True),
        ("universe1.i", "cylinder", [2, 3], True),
    ],
)
def test_extract_by_comment(universe, comment, expected, assc, universe_test_data) -> None:
    path = universe_test_data / universe
    assert path.exists(), f"Cannot find path {path}"
    actual = list(extract_cells_from_file(
        path, filter_by_comment(comment), add_surface_sharing_cells=assc
    ))
    assert set(map_names(actual)) == set(expected)
    new_universe = make_universe(actual)
    assert len(new_universe) == len(expected)
