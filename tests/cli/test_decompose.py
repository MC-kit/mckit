...
@pytest.mark.parametrize(
    "path, expected_cells",
    [
        ("cli/simple_cubes.mcnp", 3),
        ("cli/simple_cubes.universes/envelopes.i", 3),
        ("cli/simple_cubes.universes/u1.i", 2),
        ("cli/simple_cubes.universes/u2.i", 2),
    ],
)
def test_input_files_reading(path, expected_cells, data):
    universe = from_file(data / path).universe
...
def test_not_existing_mcnp_file(runner):
    result = runner.invoke(mckit, args=["decompose", "not-existing.mcnp"], catch_exceptions=False)
...
@pytest.mark.parametrize(
    "source,expected", [("cli/simple_cubes.mcnp", ["envelopes.i", "u1.i", "u2.i"])]
)
def test_when_only_source_is_specified(runner, source, expected, data):
    source = data / source
    run_result = runner.invoke(mckit, args=["decompose", str(source)], catch_exceptions=False)
...
@pytest.mark.parametrize(
    ...
    [("cli/simple_cubes.mcnp", "split-1", ["envelopes.i", "u1.i", "u2.i"])],
)
def test_when_output_is_specified(runner, source, output, expected, data):
    source = data / source
    output = Path(output)
    assert not output.exists(), f"The {output} directory should not exist before the test run"
    result = runner.invoke(
        mckit,
        args=["decompose", "--output", str(output), str(source)],
...
def test_when_output_file_exists_and_override_is_not_specified(runner, data):
    source = data / "simple_cubes.mcnp"
    with runner.isolated_filesystem() as prefix:
        output = Path(prefix) / "simple_cubes.universes/envelopes.i"
        output.parent.mkdir(parents=True)
        output.touch(exist_ok=False)
        result = runner.invoke(mckit, args=["decompose", str(source)], catch_exceptions=False)
        assert result.exit_code != 0, (
            "Should fail when output file exists and --override is not specified"
        )


def test_when_output_file_exists_and_override_is_specified(runner, data):
    source = data / "cli/simple_cubes.mcnp"
    output = Path.cwd() / "simple_cubes./envelopes.i"
    output.parent.mkdir(parents=True)
    output.touch(exist_ok=False)
    result = runner.invoke(
        mckit, args=["--override", "decompose", str(source)], catch_exceptions=False
    )
    assert result.exit_code == 0, (
...
def test_fill_descriptor(runner, data):
    source = data / "cli/simple_cubes.mcnp"
    output = Path("simple_cubes.universes/fill-descriptor.toml")
    result = runner.invoke(mckit, args=["decompose", str(source)], catch_exceptions=False)
...
def test_fill_descriptor_when_fill_descriptor_file_is_specified(runner, data):
    source = data / "cli/simple_cubes.mcnp"
    fill_descriptor_path = Path.cwd() / "fill-descriptor-special.toml"
    result = runner.invoke(
        mckit,
        args=["decompose", "--fill-descriptor", str(fill_descriptor_path), str(source)],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, "Should success"
    assert fill_descriptor_path.exists(), (
        f"Should create fill-descriptor in file {fill_descriptor_path.absolute()}"
    )


def test_anonymous_transformation(runner, data):
    source = data / "cli/cubes_with_fill_transforms.mcnp"
    output = Path.cwd() / "cubes_with_fill_transforms.universes"
    result = runner.invoke(mckit, args=["decompose", str(source)], catch_exceptions=False)
    assert result.exit_code == 0, "Should success"
...
def test_named_transformation(runner, data):
    source = data / "cli/cubes_with_fill_named_transforms.mcnp"
    output = Path.cwd() / "cubes_with_fill_named_transforms.universes"
    result = runner.invoke(mckit, args=["decompose", str(source)], catch_exceptions=False)
    assert result.exit_code == 0, "Should success"
    with (output / "fill-descriptor.toml").open("rb") as fid:
        descriptor = tomllib.load(fid)
