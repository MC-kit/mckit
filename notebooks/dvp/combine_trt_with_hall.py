...
def _(mo):
    ...
    "tokamak envelop" - пространство, занятое компонентами установки и "generated void", включающих эти компоненты  Если бы этот envelop не выходил за пределы зала, то при интеграции можно было бы оставить неизменными ячейки установки, только отбросить graveyard_in и graveyad (лучше руками, чтобы ничего не изменилось в коде MCNP). Затем добавить ячейки здания, поменяв там только ячейку зала на ее пересечение с graveyard_in установки.

    Однако, tokamak envelop выходит за пределы зала, а именно перескается с блоком "floor" (ячейка 30014). Поэтому нужно определить . ячейки, пересекающися с полом и вычесть из них все, что ниже верхней границы пола - поверхность 30033. Таких ячеек не много.
    Все они должны быть "void generated" поскольку пол опущен ниже нижнего габарита установки (на 1мм). Ячейки, не пересекающиеся с полом, нужно сохранить в модели неизменными.
    ...
    Можно отметить, что спецификации поверхностей не меняются при поиске пересечений. ППосколькуоэтому можно в интегральной модели можно использовать их исходное текстовое представление, чтобы не испытать влияние округлений параметров поверхностей, которые делает mckit.
    Поэтому по максимуму используем функцию mckit split -
...
@app.cell
def _(hall_info):
    hall_cell = hall_info.cells_index[30015]
    return (hall_cell,)
